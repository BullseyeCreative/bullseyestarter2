<?php
function bc_custom_contact_register( $wp_customize ) {
...
add_action( 'customize_register', 'bc_custom_contact_register' );

function bc_do_geocode(){
  if (get_theme_mod('regenerate_map_data')) {
    ...
    $full_addr = $address['address_1'] . $address['address_1'] . " " . $address['city'] . ", " . $address['state'] . " " . $address['zip'];
    $full_addr = urlencode($full_addr);
    $url = "http://maps.google.com/maps/api/geocode/json?address={$full_addr}";
...
add_action( 'customize_save_after', 'bc_do_geocode' );
